...
export type SectionKey =
  'personal' | 'experience' | 'education' | 'skills' | 'languages' | 'projects' | 'certifications';
...
export function countOf(parsed: ParsedCv, key: SectionKey): number {
  if (key === 'personal')
    return Object.values(parsed.personal).filter((v) => String(v ?? '').trim()).length;
...
export function applyParsed(
      ...
      mode === 'reemplazar'
        ...
        : // Al agregar, lo que ya escribiste manda sobre lo detectado.
          ({
            ...
            ...detected,
            ...Object.fromEntries(
              Object.entries(base.personal).filter(([, v]) => String(v).trim()),
            ),
          } as Profile['personal']);
  }
  ...
  if (include.has('experience') && parsed.experience.length) {
    next.experience =
      mode === 'reemplazar' ? parsed.experience : [...base.experience, ...parsed.experience];
  }
  if (include.has('education') && parsed.education.length) {
    next.education =
      mode === 'reemplazar' ? parsed.education : [...base.education, ...parsed.education];
  }
  ...
  if (include.has('languages') && parsed.languages.length) {
    next.languages =
      mode === 'reemplazar' ? parsed.languages : [...base.languages, ...parsed.languages];
  }
  if (include.has('projects') && parsed.projects.length) {
    next.projects =
      mode === 'reemplazar' ? parsed.projects : [...base.projects, ...parsed.projects];
  }
  if (include.has('certifications') && parsed.certifications.length) {
    next.certifications =
      mode === 'reemplazar'
        ? parsed.certifications
        : [...base.certifications, ...parsed.certifications];
  }
