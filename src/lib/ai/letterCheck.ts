...
const CHAT_SIGNALS: Signal[] = [
  ...
  {
    re: /\b(he revisado|veo que tienes|seg[uú]n tu perfil|revis[ée] tu)\b/i,
    label: 'comenta el perfil',
  },
  {
    re: /\b(te gustar[ií]a|quieres que|puedo redactar|puedo ayudarte|aqu[ií] tienes|necesito saber)\b/i,
    label: 'se ofrece a hacer algo',
  },
  { re: /^\s*\d+\.\s+\S/m, label: 'da una lista de pasos' },
  {
    re: /\b(para postular|pasos para|entra al sitio|rellena el formulario)\b/i,
    label: 'explica cómo postular',
  },
];
...
export function looksLikeJobList(text: string): boolean {
  const applyLinks = (
    text.match(/\b(postular|postúlate|ver oferta|apply now|ver empleo)\b/gi) ?? []
  ).length;
